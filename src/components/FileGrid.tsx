...
interface FileGridProps {
    files: FileItem[];
    onFileClick?: (file: FileItem) => void; // Appel pour informer le parent
}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {files.map((file) => (
                    <div
                        ...
                        onClick={() => {
                            if (file.type === 'folder') {
                                onFileClick?.(file); // Si dossier, appeler le parent pour gérer le popup
                            } else if (file.type === 'pdf') {
                                setSelectedFile(file); // Si fichier, ouvrir le FileDialog
                            }
                        }}
                    >
                        <div className="mb-4">
                        ...
                        <div className="flex flex-col space-y-2 text-sm text-gray-500">
                            ...
                            <div>Créé le : {file.createdAt}</div>
                            <div>Mis à jour : {file.updatedAt}</div>
                            {file.version && (
                                <div className="flex items-center space-x-2">
                  <span className="inline-flex items-center rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-medium text-orange-800">
                    v{file.version}
                                    ...
                                    <span className="text-xs">
                    ({file.versions?.length || 0} version
